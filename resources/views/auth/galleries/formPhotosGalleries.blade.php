...
<h1>Cargar imágenes</h1>
    ...
    <form method="post" enctype="multipart/form-data" action="{{ route('createPhotos') }}">
        ...
        <div id="error"></div>
        <div id="preview"></div>
        <input type="submit" id="submit" value="Guardar" class="button primary" style="display: none" />
    ...
    $('#photo').on('change', function() {
        ...
        for(let x=0; x<files.length; x++) {
            ...
            if(type !== 'image/jpeg' && type !== 'image/jpg' && type !== 'image/png') {
                ...
                $('#preview').append('<fieldset><img src="'+object_url+'"><input name="title[]" type="text" required value="" class="title" placeholder="Título: éste es el principal título de la foto (*) Mínimo 20 caracteres" /></fieldset>');
                $('input#submit').show();
            }
        }
    });
