...
<div class="dashboard-container">
    <div class="dashboard-login">
        ...
        @if (session('success'))
        <p class="alert-success"><img src="{{ asset('svg/ajax-success.svg') }}" alt="Exito"/>{{ session('success') }}</p>
        @endif
        <form method="POST" action="{{ route('login') }}">
            @csrf
            @if ($errors->has('email'))
            <p class="invalid-feedback" role="alert"><img src="{{ asset('svg/ajax-error.svg') }}" alt="Error"/>{{ $errors->first('email') }}</p>
            @endif
            @if ($errors->has('password'))
            <p class="invalid-feedback" role="alert"><img src="{{ asset('svg/ajax-error.svg') }}" alt="Error"/>{{ $errors->first('password') }}</p>
            @endif
            ...
            @endif
            <input type="submit" value="Ingresar" class="primary" />
